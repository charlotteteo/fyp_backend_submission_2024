...
class PortfolioAnalyzer:
    ...
    def calculate_metrics(self):
        ...
        downside_deviation = np.std(np.minimum(returns - target_return, 0))
        ...
        for key, value in self.portfolio_metrics.items():
            ...
            if isinstance(value, dict):
                self.portfolio_metrics[key] = {k: round(v, 2) for k, v in value.items()}
            else:
    ...
    def sectoral_exposure(self,convert_data = False):
        sector_info = {ticker: yf.Ticker(ticker).info['sector'] for ticker in self.stocks}
    ...
    def sectoral_metrics(self):
        ...
        # Sector Sharpe Ratio
        sector_sharpe_ratio = (sector_returns - risk_free_rate) / sector_volatility
        ...
        for key, value in sector_metrics.items():
            # Check if the value is a dictionary, if so, apply rounding recursively
            if isinstance(value, dict):
               sector_metrics[key] = {k: round(v, 4) for k, v in value.items()}
            else:
                # Round the value to 2 decimal places
                sector_metrics[key] = round(value, 4)
...
if __name__ == "__main__":
    ...
    portfolio_evaluator = PortfolioAnalyzer(stocks, weights, start_date, end_date, initial_investment)
    # print(portfolio_evaluator.download_data())
    print(portfolio_evaluator.get_allocation_drift_ts())
    print(portfolio_evaluator.get_correct_format_portfolio_ts())
